function [object,probe] = PINE(measurements, probe, params)
...
if params.drawFigures
    ...
    methodTitleName = "PINE " + " (\alpha = " + string(params.alpha) + ", \beta = "  + string(params.beta) +  ")";
...
for iter = 1:iterNum
    ...
    z = myDeepDenoiser(net, object, params);
    object = (1 - params.tau)*object + params.tau*z;
...
%% local functions
function out = myDeepDenoiser(net, input, params)

if params.denoiseCoord == "polar"
    outAbs = denoiseData(net, abs(input));
    % outAngle = denoiseData(net, angle(input));
    out = outAbs.*exp(1i*angle(input));
elseif params.denoiseCoord == "cartesian"
    outReal = denoiseData(net, real(input));
    outImag = denoiseData(net, imag(input));
    out = outReal + 1i*outImag;
end

end

function out = denoiseData(net, input)
minin = min(input, [], "all");
maxin = max(input, [], "all");
inputRescale = single(rescale(input)); % scaling
res = predict(net, inputRescale);
out = inputRescale - res; % DnCNN denoising
out = double((maxin-minin).*(out))+minin; % 
end
